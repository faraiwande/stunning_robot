...
class Listing(db.Model):
    ...
    price = db.Column(db.Numeric(10, 2))  # Changed from String to Numeric
    location = db.Column(db.String(100))
    description = db.Column(db.Text)
    category = db.Column(db.String(50))  # Added product categorization
    views = db.Column(db.Integer, default=0)  # Added popularity tracking
...
class Payment(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid4()))
    seller_phone = db.Column(db.String(20), db.ForeignKey('sellers.phone'), nullable=False)
    amount = db.Column(db.Numeric(10, 2))  # Changed from Numeric to specific precision
